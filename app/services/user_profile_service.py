...
def find_matching_candidates(
        ...
        if use_vector:
            cur.execute(
                ...
                SELECT
                    ...
                    up.created_at,
                    CASE
                        WHEN up.skills_embedding IS NOT NULL
                        THEN 1 - (up.skills_embedding <=> %s::vector)
                        ELSE 0.0
                    END AS vector_score,
                    u.id AS user_id,
                    up.profile_slug,
                ...
                ORDER BY
                    CASE
                        WHEN up.skills_embedding IS NOT NULL
                        THEN up.skills_embedding <=> %s::vector
                        ELSE 1.0
                    END
                LIMIT %s
                """,
    ...
    for row in rows:
        if len(row) >= 11:
            (pid, email, full_name, skills, cv_filename, created_at,
             vector_score, user_id, profile_slug, location, years_experience) = row
        else:
            (pid, email, full_name, skills, cv_filename, created_at,
             vector_score, user_id, profile_slug) = row
            location = None
